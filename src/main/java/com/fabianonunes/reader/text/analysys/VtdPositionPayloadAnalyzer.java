...
public class VtdPositionPayloadAnalyzer extends Analyzer {
	...
	public TokenStream tokenStream(String fieldName, Reader reader) {
		...
		if (fieldName.equals("page")) {
			...
			stream = new WhitespaceTokenizer(reader);
		...
		if (fieldName.equals("content")) {
			try {
				stream = new VtdPositionTokenizer(reader);
			} catch (Exception e) {
				stream = new WhitespaceTokenizer(reader);
			}
		} else {
		...
		stream = new EnumeratorFilter(stream);
